    ...
    <div style="max-width: 400px; margin: 40px auto; padding: 32px 24px; border-radius: 8px;">
        ...
        <form method="POST" action="{{ route('users.profile.update') }}" enctype="multipart/form-data">
            ...
            <div style="margin-bottom: 16px;">
                ...
                <input type="text" name="postal_code" value="{{ old('postal_code', $address->postal_code ?? '') }}" style="width: 100%; border: 1px solid #ccc; border-radius: 6px; padding: 8px; margin-top: 4px;">
                @error('postal_code')
                    <p style="color: #FF6F6F; font-size: 0.85rem;">{{ $message }}</p>
                @enderror
            </div>
            <div style="margin-bottom: 16px;">
                ...
                <input type="text" name="address" value="{{ old('address', $address->address ?? '') }}" style="width: 100%; border: 1px solid #ccc; border-radius: 6px; padding: 8px; margin-top: 4px;">
                @error('address')
                    <p style="color: #FF6F6F; font-size: 0.85rem;">{{ $message }}</p>
                @enderror
            </div>
            <div style="margin-bottom: 32px;">
                ...
                <input type="text" name="building" value="{{ old('building', $address->building ?? '') }}" style="width: 100%; border: 1px solid #ccc; border-radius: 6px; padding: 8px; margin-top: 4px;">
                @error('building')
                    <p style="color: #FF6F6F; font-size: 0.85rem;">{{ $message }}</p>
                @enderror
            </div>
